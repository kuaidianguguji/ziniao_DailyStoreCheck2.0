...
- TikTok 和 Shopee 只接管紫鸟已经打开的当前页面；美客多会在同一个紫鸟标签页内进入经营指标页，不会创建普通浏览器。
...
- `MKD_auto.py` 先读取紫鸟当前标签页网址：网址包含 `vendedores` 时进入卖家后台经营指标页，否则进入 `https://www.mercadolivre.com.br/metricas#sc-menu`；`PERIOD_CLICK_STEPS` 负责点击日期按钮、最近 7 天和最近 30 天选项，最后一个日期选项点击后固定等待 30 秒再读取数据。
...
- TikTok 接管后不等待 `document.readyState=complete` 或固定 10 秒；登录入口出现时处理登录，营销按钮或店铺广告按钮出现时立即进入采集。点击店铺广告后以“成本”指标可见确认广告页加载完成，点击概览后以 GMV 指标可见确认概览页加载完成；每次切换昨天/7天后也必须重新确认对应指标可见。
...
- TikTok 同一日期周期的全部指标通过一次页面 JavaScript 批量 XPath 读取；批量脚本异常时才回退为逐项读取，字段转换和原始值日志仍逐项保留。
