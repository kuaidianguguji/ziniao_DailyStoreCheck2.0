...
- Shopee 采集昨天和最近 7 天各 12 个 ALL 指标。巴西金额如 `R$18.558,26` 转为 `18558.26`，广告支出回报率如 `7,61` 转为 `7.61`；优惠价金额、优惠券带来销售额使用金额字段，展示次数、点击数、订单量、商品已出售和加购次数写入数值字段，加购率和点击率按飞书“小数”字段写入去掉百分号后的两位数值；缺失 XPath、元素或无效文本均返回空值并记录原因。
