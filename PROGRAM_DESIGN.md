...
- Shopee 每次打开日期面板前，都会把时间切换按钮滚动到浏览器视口中心，再执行点击和状态验证；日期切换必须等到“昨天/最近 7 天”选项出现才认定面板打开成功，选择日期后选项需要连续两次不可见才认定选择成功，随后等待 30 秒再读取 ALL 行指标。
- Shopee 采集昨天和最近 7 天各 12 个 ALL 指标。巴西金额如 `R$18.558,26` 转为 `18558.26`，广告支出回报率如 `7,61` 转为 `7.61`；优惠价金额、优惠劵带来销售额使用 BRL，展示次数、点击数、订单量、商品已出售和加购次数为整数，加购率和点击率为百分比；缺失 XPath、元素或无效文本均返回空值并记录原因。
- Shopee 日志记录菜单可见性判断、每次按钮查找/点击/重试、日期面板出现和消失、指标原始值与转换值及其 Python 类型，以及最终打包 JSON。当前未提供 SP 多维表字段结构，因此暂时按“一项指标一条通用记录”写入飞书。
...
5. TikTok 短期表按本项目约定建立 33 个字段；Shopee 和 Mercado 可继续使用店铺名、采集时间、指标、数值、原始数据及各平台扩展字段。
6. 多维表“采集时间”使用日期字段，程序写入前统一转换为毫秒时间戳。
7. TikTok 历史电子表第一行应按 `TIKTOK_TABLE_FIELD_ORDER` 的 33 字段顺序建立；Shopee 和 Mercado 电子表可使用店铺名、采集时间、平台、指标、数值、原始数据及扩展字段。
